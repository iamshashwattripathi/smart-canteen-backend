 ...
 *
 * POST /api/chat/message — Send message, receive AI reply GET
 * /api/chat/history/{sid} — Get conversation history for a session DELETE
 * /api/chat/session/{sid} — Clear/reset a chat session
 *
 * All endpoints are public — guests and logged-in users can both chat.
 *
 * Frontend integration: 1. Generate a UUID as sessionId (store in localStorage)
 * 2. POST { sessionId, message, pageContext } → receive { reply, sessionId,
 * timestamp } 3. Display the AI reply in your chatbot widget 4. Pass
 * pageContext from your router (e.g. "menu", "cart", "orders", "track",
 * "wallet")
 */
@RestController
...
public class AIChatbotController {
	 ...
	 *
	 * Request body: { "sessionId": "uuid-string", // unique per user/browser tab
	 * "message": "What's for lunch?", // user's message "pageContext": "menu" //
	 * current page (home/menu/cart/orders/track/wallet/admin) }
	 *
	 * Response: { "reply": "Here are today's highlights...", "sessionId":
	 * "uuid-string", "timestamp": "2025-05-01T12:00:00" }
	 */
	@PostMapping("/message")
	public ResponseEntity<Map<String, Object>> message(@RequestBody Map<String, Object> body) {
		...
		return ResponseEntity
				.ok(Map.of("reply", reply, "sessionId", sessionId, "timestamp", LocalDateTime.now().toString()));
	}

	/**
	 * Get full conversation history for a session. Useful to restore chat widget
