 ...
 * — Core DSA Component
 *
 * Implements real-time stall load balancing using a Java PriorityQueue
 * (min-heap). Each active stall is inserted with its current queue length as
 * the priority key. When a new order arrives, the stall at the top of the heap
 * (smallest queue length) is selected, its count is incremented, and the heap
 * is re-ordered — this is the "Queue Optimization" feature described in the
 * project synopsis.
 *
 * Time complexity: assignStall() → O(log n) where n = number of stalls
...
public class QueueService {
	...
	public Stall assignLeastBusyStall() {
		...
		PriorityQueue<Stall> stallHeap = new PriorityQueue<>(Comparator.comparingInt(Stall::getCurrentQueueLength));
	...
	/**
	 * Called when an order is marked COMPLETED or CANCELLED. Decrements the stall's
	 * queue counter so the heap stays accurate.
	 */
	...
	/**
	 * Estimates the pickup time for a new order based on: pickupTime = now +
	 * (queueLength * avgPrepTimeMinutes)
	 *
	...
	/**
	 * Returns a snapshot of all stall queue lengths. Used by the admin dashboard to
	 * display real-time load.
	 */
	public Map<String, Integer> getQueueSnapshot() {
		Map<String, Integer> snapshot = new LinkedHashMap<>();
		stallRepository.findByActiveTrue().forEach(s -> snapshot.put(s.getName(), s.getCurrentQueueLength()));
		return snapshot;
	}
}
