...
public class NotificationService {
	...
	public Notification notify(User user, Order order, String message) {
		Notification notification = Notification.builder().user(user).order(order)
				.type(Notification.NotificationType.PUSH).message(message).status(Notification.NotificationStatus.SENT)
				.build();
		...
		try {
			messagingTemplate.convertAndSend("/topic/notifications/" + user.getId(),
					Map.of("id", saved.getId(), "message", saved.getMessage(), "orderId",
							order != null ? order.getId() : null, "orderCode",
							order != null ? order.getOrderCode() : null, "sentAt", saved.getSentAt().toString()));
		} catch (Exception e) {
	...
	public void markAllRead(Long userId) {
		List<Notification> unread = notificationRepository.findByUser_IdAndIsReadFalseOrderBySentAtDesc(userId);
	...
	public void broadcastOrderStatus(String orderCode, String status, String pickupTime) {
		try {
			messagingTemplate.convertAndSend("/topic/order/" + orderCode, Map.of("orderCode", orderCode, "status",
					status, "pickupTime", pickupTime != null ? pickupTime : ""));
		} catch (Exception e) {
