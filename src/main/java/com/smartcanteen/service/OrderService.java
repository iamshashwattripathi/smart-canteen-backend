...
public class OrderService {
	...
	// ── Place a new order ────────────────────────────────────
	@Transactional
	public Order placeOrder(Long userId, List<Map<String, Object>> cartItems, String paymentMethod) {
		...
		// 2. Build order shell
		Order order = Order.builder().orderCode(generateOrderCode()).user(user).stall(assignedStall)
				.status(OrderStatus.PENDING).paymentMethod(PaymentMethod.valueOf(paymentMethod.toUpperCase()))
				.paymentStatus(PaymentStatus.UNPAID).createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now())
				.build();
		...
		for (Map<String, Object> cartItem : cartItems) {
			...
			OrderItem oi = OrderItem.builder().order(order).menuItem(menuItem).quantity(quantity)
					.unitPrice(menuItem.getPrice()).subtotal(itemSubtotal).build();
		...
		BigDecimal gstAmount = subtotal.multiply(BigDecimal.valueOf(gstRate)).divide(BigDecimal.valueOf(100), 2,
		...
		int nextToken = tokenRepository.findMaxTokenToday().map(t -> t + 1).orElse(1);

		Token token = Token.builder().tokenNumber(nextToken).order(savedOrder).stall(assignedStall)
				.status(Token.TokenStatus.WAITING).issuedAt(LocalDateTime.now()).build();
		...
		String eta = savedOrder.getPickupTime() != null ? savedOrder.getPickupTime().toLocalTime().toString() : "soon";
		notificationService.notify(user, savedOrder,
				"Your order " + savedOrder.getOrderCode() + " has been placed! Token #" + nextToken + " at "
						+ assignedStall.getName() + ". Estimated pickup: " + eta);
	...
	public Order updateStatus(Long orderId, String newStatus) {
		...
		notificationService.broadcastOrderStatus(order.getOrderCode(), status.name(),
