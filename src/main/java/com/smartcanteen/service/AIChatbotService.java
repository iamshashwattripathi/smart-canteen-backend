...
public class AIChatbotService {
	...
	public String chat(String sessionId, String userMessage, String pageContext) {
		...
		// 1. Save user message
		chatMessageRepository.save(ChatMessage.builder().sessionId(sessionId).role(ChatMessage.MessageRole.USER)
				.content(userMessage).pageContext(pageContext).build());

		// 2. Build conversation history (last 20 messages)
		List<ChatMessage> history = chatMessageRepository.findTop20BySessionIdOrderByCreatedAtAsc(sessionId);

		// 3. Call Gemini API
		String reply = callGeminiAPI(history, pageContext);

		// 4. Save assistant reply
		chatMessageRepository.save(ChatMessage.builder().sessionId(sessionId).role(ChatMessage.MessageRole.ASSISTANT)
				.content(reply).pageContext(pageContext).build());
	...
	private String callGeminiAPI(List<ChatMessage> history, String pageContext) {
		...
		try {
			...
			requestBody.put("system_instruction", Map.of("parts", List.of(Map.of("text", systemText))));
	...
	private String buildMenuSummary() {
		try {
			List<MenuItem> items = menuItemRepository.findAll().stream().filter(MenuItem::isAvailable)
					.collect(Collectors.toList());

			Map<String, List<String>> grouped = new LinkedHashMap<>();
			for (MenuItem item : items) {
				grouped.computeIfAbsent(item.getCategory(), k -> new ArrayList<>()).add(item.getName() + " ₹"
						+ item.getPrice().toPlainString() + (item.isVeg() ? " [Veg]" : " [Non-Veg]"));
			}
	...
	private String buildQueueInfo() {
		try {
			...
			return snapshot.entrySet().stream().map(e -> e.getKey() + ": " + e.getValue() + " orders waiting")
